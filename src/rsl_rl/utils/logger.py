...
class Logger:
    ...
    def init_logging_writer(self) -> None:
        ...
        if self.log_dir is not None and not self.disable_logs:
            ...
            if self.logger_type == "neptune":
                from src.rsl_rl.utils.neptune_utils import NeptuneSummaryWriter

                self.writer = NeptuneSummaryWriter(log_dir=self.log_dir, flush_secs=10, cfg=self.cfg)
            elif self.logger_type == "wandb":
                from src.rsl_rl.utils.wandb_utils import WandbSummaryWriter
    ...
    def log(
        ...
        if self.writer is not None:
            ...
            # Log losses and custom metrics
            loss_items: list[tuple[str, float, str]] = []
            for key, value in loss_dict.items():
                if "/" in key:
                    self.writer.add_scalar(key, value, it)  # type: ignore
                    loss_items.append((key, value, "metric"))
                else:
                    self.writer.add_scalar(f"Loss/{key}", value, it)
                    loss_items.append((key, value, "loss"))
            ...
            # Print losses and AMP metrics
            amp_string = ""
            for key, value, kind in loss_items:
                if key.startswith("AMP/"):
                    amp_string += f"""{f"{key.split('/', 1)[1]}:":>{pad}} {value:.4f}\n"""
                elif kind == "loss":
                    log_string += f"""{f"Mean {key} loss:":>{pad}} {value:.4f}\n"""
            if amp_string:
                log_string += f"\033[1m{' AMP metrics ':-^{width}}\033[0m\n{amp_string}"
