...
class MotionResetManager:
    ...
    def init(
        ...
        expected_joint_names = list(robot.joint_names)
        for motion in loader.motion_data + loader.motion_data_recovery:
            if motion["joint_names"] != expected_joint_names:
                raise ValueError(
                    f"{motion['motion_name']}: motion joint order does not match "
                    "the current robot; regenerate AMP motions"
                )
