...
def _normalize_quat_wxyz(quat: np.ndarray) -> np.ndarray:
  ...
  flip = q[..., :1] < 0.0
  return np.where(flip, -q, q).astype(np.float32)
...
class MotionLoader:
    ...
    def _load_dir(dir_path: str, device: str) -> list[dict]:
        ...
        for filename in sorted(os.listdir(dir_path)):
            if not filename.endswith(".npz") or filename.endswith("_M.npz"):
                continue
            ...
            if "joint_names" in data:
              ...
              raise ValueError(
                f"{filename}: missing joint_names metadata; "
                "run scripts/prepare_amp_motions.py --mode stamp"
              )
            body_quat_w = _normalize_quat_wxyz(data["body_quat_w"])
            result.append({
